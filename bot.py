...
def sheet_update(ws, chat_id, email, status, notes=""):
  ...
  for i, r in enumerate(rows[1:], start=2):
    if len(r)>=4 and r[1]==str(chat_id) and r[3].strip().lower()==email.strip().lower():
      r[5] = status
      if len(r)>=7: r[6] = ((r[6]+" | ") if r[6] else "") + (notes or "")
...
async def call_create_demo(name, email, password, phone, country="Serbia"):
  return await _post_json(f"{PUP_URL}/create-demo", {"name":name,"email":email,"password":password,"phone":phone,"country":country})

async def call_create_mt4(email, password):
  return await _post_json(f"{PUP_URL}/create-mt4", {"email":email,"password":password})

async def start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
  await update.message.reply_text("Zdravo! Kako se zoveš? (npr. Marko)"); return ASK_NAME

async def got_name(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
  name=update.message.text.strip()
  if len(name)<2: await update.message.reply_text("Prekratko ime, probaj opet:"); return ASK_NAME
  ctx.user_data["name"]=name; await update.message.reply_text("Super! Unesi svoj email:"); return ASK_EMAIL

async def got_email(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
  email=update.message.text.strip()
  if not EMAIL_REGEX.match(email): await update.message.reply_text("Email nije validan. Unesi ponovo:"); return ASK_EMAIL
  ctx.user_data["email"]=email; await update.message.reply_text("Unesi broj telefona (sa pozivnim, npr. +381641234567 ili 064...):"); return ASK_PHONE

async def got_phone(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
  raw_phone = update.message.text.strip()
  phone = _norm_phone(raw_phone, "+381")
  if len(phone)<8:
    await update.message.reply_text("Telefon nije validan. Probaj ponovo (npr. +38164xxxxxxx):"); return ASK_PHONE

  name  = ctx.user_data["name"]; email = ctx.user_data["email"]
  pwd   = f"{name}123#"; chat_id = update.effective_chat.id
  ws = _ws()
  if sheet_email_exists(ws, email): await update.message.reply_text("Taj email je već registrovan. Unesi drugi:"); return ASK_EMAIL
  sheet_add(ws, chat_id, name, email, pwd, status="pending", notes=f"phone:{phone}")

  await update.message.reply_text(f"✅ Hvala, {name}! Kreiram tvoj DEMO... Sačekaj 10–30 sekundi.")

  # DEMO — ali se ponašamo optimistički (zbog zaštite)
  ok, data = await call_create_demo(name, email, pwd, phone, country="Serbia")
  shots = (data or {}).get("screenshots", [])
  if shots:
    try: await update.message.reply_photo(shots[-1], caption="📸 Outcome screenshot")
    except: pass

  # Treat as created (često realno bude kreiran i kada endpoint vrati error/timeout)
  if ok and data.get("ok"):
    sheet_update(ws, chat_id, email, "created", data.get("note",""))
    await update.message.reply_text("🎉 Demo je kreiran! Pokušavam da kreiram MT4 nalog…")
  else:
    sheet_update(ws, chat_id, email, "maybe_created", data.get("note") or data.get("error") or "guard/timeout")
    await update.message.reply_text("🎉 Demo (verovatno) kreiran. Pokušavam da kreiram MT4 nalog…")

  # MT4
  mt4_ok, mt4 = await call_create_mt4(email, pwd)
  if mt4_ok and mt4.get("ok") and mt4.get("mt4_login"):
    ...
    await update.message.reply_text(f"MetaTrader 4 Login: {mt4_login}\nŠifra: {pwd}\n\nAko zapne — javi se SUPPORT-u ⬇️", reply_markup=kb)
  else:
